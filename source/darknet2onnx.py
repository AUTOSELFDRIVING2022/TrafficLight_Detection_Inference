...
def transform_to_onnx(cfgfile, weightfile, batch_size=1, dynamic, onnx_file_name):
...
if __name__ == '__main__':
    if len(sys.argv) == 6:
        ...
        onnx_file_name = sys.argv[5]
        transform_to_onnx(cfgfile, weightfile, batch_size, dynamic, onnx_file_name)
    else:
        print('Please execute this script this way:\n')
        print('  python darknet2onnx.py <cfgFile> <weightFile>')
        print('or')
        print('  python darknet2onnx.py <cfgFile> <weightFile> <batchSize> <onnx_file_name>')
