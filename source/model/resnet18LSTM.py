...
class BasicBlock(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # if downsample required -> downsample layer must be in args 
...
class ResNetLSTM(nn.Module):
    def __init__(
        ...
        norm_layer: Optional[Callable[..., nn.Module]] = None, 
        lstm_hidden_layer = 512,
        lstm_sequence_number = 10
    ) -> None:
        super(ResNetLSTM, self).__init__()

        # LSTM sequence layer
        self.nseq = lstm_sequence_number
        ...
        self.lstm = nn.LSTM(
            input_size = 512, hidden_size=lstm_hidden_layer, batch_first=True)
        
        self.fc = nn.Linear(lstm_hidden_layer, num_classes)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = self.fc(ht[-1])
        #print('fc shape after LSTM:', x.shape)
        return x

if __name__ == "__main__":
    model = ResNetLSTM(BasicBlock, [2, 2, 2, 2], num_classes = 7, lstm_hidden_layer = 512, lstm_sequence_number = 10)
    x = torch.randn(20, 10, 3, 112, 112)
    y = model(x)
